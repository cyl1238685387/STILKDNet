...
from models.losses import cross_entropy, FocalLoss,BoundaryLoss,Boundary_ce_loss,Focal_Dice_BL,FocalLoss_with_dice,BCEDiceLoss,NewBCEDiceLoss
...
def strong_argument(img):

    # 颜色抖动增强，设置合适的参数来控制增强强度，这里示例参数可按需调整
    color_jitter = ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1)
    img = color_jitter(img)

    # 随机高斯模糊增强，默认核大小和sigma范围，也可按需修改参数
    gaussian_blur = GaussianBlur(kernel_size=(3, 3), sigma=(0.1, 2.0))
    img = gaussian_blur(img)
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.New_pxl_loss = NewBCEDiceLoss
    ...
    def _forward_pass(self, args,batch):
        ...
        if args.loss_SD== True :
            ...
            # img_in2_w=weak_argument(img_in2)
            # self.G_pred_w = self.net_G(img_in1, img_in2_w)

            img_in2_s=strong_argument(img_in2)
            self.G_pred_s = self.net_G(img_in1, img_in2_s)
    ...
    def _backward_G(self,args):
        ...
        if args.loss_SD == True:
            ...
            if args.deep_supervision == True:
                ...
                #teacher
                self.G_lossno = self._pxl_loss(self.G_pred, gt)
                #student
                # self.G_loss_w = self._pxl_loss(self.G_pred_w, gt.detach())


                self.G_loss_s = self._pxl_loss(self.G_pred_s, gt)

                self.G_loss_so = self.New_pxl_loss(self.G_pred_s, self.G_pred)

                self.G_loss=self.G_lossno+0.9*self.G_loss_s+0.1*self.G_loss_so


                # self.G_lossno = self._pxl_loss(self.G_pred, gt)
                # self.G_loss_w = self._pxl_loss(self.G_pred_w, gt)
                # self.G_loss_s = self._pxl_loss(self.G_pred_s, gt)


                #  # 定义可学习的权重参数
                # weights = torch.nn.Parameter(torch.tensor([1/3, 1/3, 1/3], requires_grad=True))
                #  # 归一化权重，确保权重总和为1
                # weights_normalized = nn.functional.softmax(weights, dim=0)

                # self.G_loss = (weights_normalized[0] *self.G_lossno  +
                #     weights_normalized[1] * self.G_loss_w +
                #     weights_normalized[2] * self.G_loss_s )
